...
![Web学习](https://i0.hdslb.com/bfs/article/7cb9adce218afe6309576542a338f0fc09334d58.png@942w_2670h_progressive.webp "Web学习")
...
background-attachment; scroll / scroll / local; /* 是否滚动 */
...
FormatDate(date) {
	...
	let month = MakeUp(dates.getMonth() + 1); //月
	let day = MakeUp(dates.getDate()); //日
	let week = weekFormat(dates.getDay()); //星期
	let hour = MakeUp(dates.getHours()); //小时
	let minute = MakeUp(dates.getMinutes()); //分钟
	let second = MakeUp(dates.getSeconds()); //秒
...
var date = new Date();
FormatDate(date);	// 2021-06-07 15:00
```
...
var obj = {};
for (var item in arr){	//obj = { name1: '1', name2: '2', name3: '3' }
...
var arr = [];
for (var item in obj){	//arr = [ 'name1', 20 ]
	arr.push(obj[item]);
};
for (var item in obj){	//arr = [ 'name', 'age' ]
    arr.push(item);
};
```
...
- **BFD UI**    [文档链接](http://ui.baifendian.com/)
...
- **Quasar**    [文档链接](http://www.quasarchs.com/)
- **Ant Design of Vue**    [文档链接](https://antdv.com/docs/vue/introduce-cn/)
- **Echarts(数据可视化)**    [文档链接](https://echarts.apache.org/zh/index.html)
- **highCharts(数据可视化)**    [文档链接](https://www.highcharts.com/)
